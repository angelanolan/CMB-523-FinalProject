...
# Define the start and end positions of exon 1 (hypothetical values because i havent figured out the actul values)
exon1_start <- 100
exon1_end <- 1000
...
# Define primer design criteria
primer_length <- 20
gc_content_range <- c(15, 90)

# Generate candidate primers from the exon 1 sequence
candidate_primers <- DNAStringSet(sample(exon1_sequence, size = 100, replace = TRUE))

# Filter candidate primers based on criteria
filtered_primers <- candidate_primers[
  nchar(candidate_primers) == primer_length &
  sum(candidate_primers == "G" | candidate_primers == "C") / primer_length * 10 >= gc_content_range[1] &
  sum(candidate_primers == "G" | candidate_primers == "C") / primer_length * 10 <= gc_content_range[2]
]
